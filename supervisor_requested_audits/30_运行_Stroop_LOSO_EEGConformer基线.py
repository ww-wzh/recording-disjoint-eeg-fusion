...
OUTPUT_ROOT = HERE / "31_输出_Stroop_LOSO_EEGConformer基线"
CHECKPOINT_ROOT = OUTPUT_ROOT / "01_检查点_可续跑"
CACHE_ROOT = ROUTE_A_ROOT / "cache" / "raw_8ch_bandpass_0p5_45_v1"

RAW_OUTPUT = OUTPUT_ROOT / "02_结果_EEGConformer窗口种子预测.csv"
RECORDING_OUTPUT = OUTPUT_ROOT / "03_结果_EEGConformer五种子Recording预测.csv"
SUBJECT_OUTPUT = OUTPUT_ROOT / "04_结果_EEGConformer参与者准确率.csv"
SUMMARY_OUTPUT = OUTPUT_ROOT / "05_结果_EEGConformer汇总.csv"
...
def main() -> None:
    if not torch.cuda.is_available():
        raise RuntimeError(
            "EEG-Conformer 正式实验需要 CUDA，但当前环境 torch.cuda.is_available() 为 False"
        )
    ...
    print("[1/3] 运行 15 名参与者 x 5 种子的 fully nested Stroop LOSO EEG-Conformer")
    for subject in range(1, 16):
        ...
        if subject in set(source.subjects.astype(int)):
            raise AssertionError("外层目标参与者进入了 EEG-Conformer 训练数据")
        splits = subject_loso_folds(source.subjects)
        if len(splits) != 14:
            raise AssertionError("每个外层 EEG-Conformer 单元必须包含 14 折内层 participant-LOSO")
        for seed in SEEDS:
            if checkpoint_complete(subject, seed, protocol_digest):
                print(f"[skip] EEG-Conformer S{subject:02d} seed={seed}")
                continue
            print(f"[start] EEG-Conformer S{subject:02d} seed={seed}")
            ...
            if not checkpoint_complete(subject, seed, protocol_digest):
                raise AssertionError(
                    f"EEG-Conformer S{subject:02d} seed={seed} 写入后未通过检查"
                )
            print(f"[done] EEG-Conformer S{subject:02d} seed={seed}")
    ...
    if not all(
        ...
        raise RuntimeError("75 个 EEG-Conformer 外层参与者-种子单元尚未全部完成")
    ...
    if raw.duplicated(keys).any():
        raise AssertionError("合并后的 EEG-Conformer 窗口预测存在重复键")
    ...
    if len(recordings) != 60:
        raise AssertionError(
            f"EEG-Conformer 最终 Recording 结果应有 60 行，实际为 {len(recordings)} 行"
        )
    ...
    print(f"EEG-Conformer Stroop LOSO 完成，输出目录：{OUTPUT_ROOT}")
